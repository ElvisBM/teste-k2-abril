...
class add_preco{
    ...
    static function metabox(){
        ...
        if(!isset($metadata['preco'])){
           $link = "";
        }else{
            $link = $metadata['preco'][0];
        };
        ?>
        <label>Digite o preço: </label>
        <input type="text" name="<?php echo __CLASS__ ?>[preco]" value="<?php echo  $link; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;" />
        <?php
    }
