...
class add_telefone{
    ...
    static function metabox(){
        ...
        if(!isset($metadata['telefone'])){
           $link = "";
        }else{
            $link = $metadata['telefone'][0];
        };
        ?>
        <label>Digite o telefone</label>
        <input type="text" name="<?php echo __CLASS__ ?>[telefone]" value="<?php echo  $link; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;" />
        <?php
    }
